...
const fetchAlphaVantageCandlestickData = async (
  ...
  try {
    ...
    // Use the existing Yahoo Finance function
  ...
  try {
    ...
    if (onChainData) {
      return {
        ...
        volume: onChainData.volume || 0,
        rating: 5, // Default
        trend: 'NEUTRAL', // Not enough data for trend
        analysis: 'Analysis based on blockchain data',
        recommendation: 'HOLD' // Default recommendation with limited data
      };
    }
...
export const fetchTrendingAssets = async (): Promise<TrendingAsset[]> => {
  ...
  try {
    ...
    assets.filter(asset => asset !== null).forEach(asset => {
      if (asset) { // Type guard to make TypeScript happy
        trendingAssets.push({
          id: asset.id,
          symbol: asset.symbol,
          name: asset.name,
