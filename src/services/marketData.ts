...
const fetchCryptoData = async (symbol: string): Promise<Asset | null> => {
  try {
    ...
    return await fetchOnChainAnalytics(symbol);
  } catch (error: any) {
...
/**
 * Fetches candlestick data from Alpha Vantage
 * Uses demo API key with limited functionality
 */
const fetchAlphaVantageCandlestickData = async (
  ...
  timeframe: '30d' | '90d' | '1y' = '90d'
      ...
      console.error('Error scraping Yahoo Finance for crypto:', yahooError);
    }
 ...
 * Enhanced with multiple web scraping approaches for maximum data coverage
 */
const scrapeCandlestickData = async (
  ...
  try {
    console.log(`Attempting to scrape candlestick data for ${symbol}`);
    
    // Try multiple web sources in parallel for better reliability
    const scrapingPromises = [
      // Yahoo Finance (primary source)
      scrapeYahooFinanceData(symbol, timeframe),
      
      // For stocks, try Investing.com as an additional source
      isStock ? scrapeInvestingData(symbol, timeframe) : Promise.resolve([]),
      
      // For crypto, try additional sources
      !isStock ? scrapeCryptoData(symbol, timeframe) : Promise.resolve([])
    ];
    ...
    results.forEach((result, index) => {
      if (result.status === 'fulfilled' && result.value.length > 0) {
        const source = index === 0 ? 'Yahoo Finance' : 
                      index === 1 ? 'Investing.com' : 'Crypto Source';
        
        console.log(`Successfully scraped ${result.value.length} data points from ${source}`);
        
        result.value.forEach(candle => {
          if (!dataMap.has(candle.date)) {
            dataMap.set(candle.date, candle);
          }
        });
    ...
    console.log(`Total scraped data points: ${combinedData.length}`);
    return combinedData;
  } catch (error) {
 ...
 */
const scrapeYahooFinanceData = async (
  symbol: string,
  ...
  try {
    ...
    console.error('Error scraping Yahoo Finance data:', error);
    return [];
  }
};

/**
 * Scrapes stock data from Investing.com
 */
const scrapeInvestingData = async (
  symbol: string,
  timeframe: '30d' | '90d' | '1y'
): Promise<CandlestickData[]> => {
  try {
    // This would be a more sophisticated web scraping implementation
    // For a real implementation, we would use a headless browser or HTML parsing
    // For now, we'll return an empty array as this is just a placeholder
    console.log('Investing.com scraping would be implemented here');
    return [];
  } catch (error) {
    console.error('Error scraping Investing.com data:', error);
    return [];
  }
};

/**
 * Scrapes crypto data from various sources
 */
...
// This function has been removed as we're no longer using mock data
